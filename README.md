...
| **Auth** | `api/auth/` | bcrypt + JWT, role hierarchy + peer roles, dev-mode bypass |
| **Kafka consumers** | `ingestion/kafka_consumers/` | Transaction, SafeGuard, SIM-swap, device events |
| **WebSocket feeds** | `api/websocket/` | 6 feeds bridged via Redis pub/sub (3 fixed + per-takedown pattern) |
| **REST API** | `api/routes/` | 80+ endpoints across dashboard, clusters, nodes, agents, alerts, takedowns, rules, analytics, LE, integration |
...
by three core WS feeds (alerts, cluster updates, metrics) plus three
derived feeds (rules, integration, per-takedown) — all bridged through
Redis pub/sub so any producer (route, Celery task, consumer) can
broadcast without knowing who's listening.
...
| WebSocket | `/ws/alerts`, `/ws/cluster-updates`, `/ws/metrics`, `/ws/rules`, `/ws/integration`, `/ws/takedown/{id}` |
